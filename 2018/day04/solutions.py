...
# Part one
guard1 = max(total.items(), key=lambda x: x[1])[0]
minute1 = Counter(minute[guard1]).most_common(1)[0][0]
print(guard1 * minute1)

# Part two
guard2 = max(minute.items(), key=lambda m: Counter(m[1]).most_common()[0][1])[0]
minute2 = Counter(minute[guard2]).most_common(1)[0][0]
